...
public class Library {
    ...
    public void addBook(String title, String author, int year) {
        ...
        if (catalog.containsKey(book.getBookName())) {
            catalog.get(book.getBookName()).add(book);
        } else {
            List<Book> books = new ArrayList<>();
            books.add(book);
            catalog.put(book.getBookName(), books);
        }
    }
    ...
    public List<Book> searchByAuthor(String author) {
        ...
        for (List<Book> books : catalog.values()) {
            for (Book book : books) {
                if (book.getAutorName().equals(author)) {
                    result.add(book);
                }
            }
        }
    ...
    public boolean borrowBook(Book book, Reader reader) {
        if (catalog.containsKey(book.getBookName())) {
            List<Book> books = catalog.get(book.getBookName());
